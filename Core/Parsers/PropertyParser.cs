    ...
    public class PropertyParser
    {
        ...
        private string GenerateCustomName(Type type)
        {
            ...
            else if (type.GetInterfaces().Any(x => x.Name == "IEnumerable"))
            {
                ...
                {
                    var types = System.Reflection.Assembly.GetExecutingAssembly().GetTypes().Where(x => x.GetInterfaces().Any(tp => tp == typeof(Core.Models.IToConvertSearchable))).ToList();
            ...
            {
                var types = Assemply.GetTypes().Where(x => x.GetInterfaces().Any(tp => tp.Name == typeof(Core.Models.IToConvertSearchable).Name)).ToList();
